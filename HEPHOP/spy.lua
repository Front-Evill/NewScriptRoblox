...
CodeDisplay.Text = "<font color=\"#7A828B\">-- En attente d'أ©vأ©nements...\n-- Les scripts apparaأ®tront ici avec la coloration syntaxique.</font>"
...
local function getPathToInstance(instance)
    ...
    for i, inst in ipairs(path) do
        ...
        if i == 1 then
            -- Rأ©cupأ¨re le service de maniأ¨re propre (ex: game:GetService("ReplicatedStorage"))
            pathString = string.format("game:GetService(%q)", name)
        else
            -- Si le nom contient des caractأ¨res spأ©ciaux, on utilise ["nom"] au lieu de .nom
...
local function formatValue(value, depth)
    depth = depth or 0
    if depth > 8 then return '"[Table trop profonde]"' end -- Sأ©curitأ© anti dأ©bordement de pile
...
CopyButton.MouseButton1Click:Connect(function()
    if setclipboard then
        if _G.RawCode ~= "" then
            ...
            CopyButton.Text = "Copiأ© !"
            task.wait(1.5)
            CopyButton.Text = "Copier"
        end
    else
        warn("Ton exأ©cuteur ne supporte pas setclipboard")
    end
end)
...
print("CENIROSO SPY V1.8 chargأ© avec succأ¨s (Optimisأ© & Sأ©curisأ©).")
